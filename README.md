...
Trong **Cài đặt > Cập nhật ứng dụng**, nhập repository công khai theo dạng `ten-tai-khoan/ten-repository`. Người dùng có thể bấm **Kiểm tra cập nhật** hoặc bật tự động kiểm tra. Updater chỉ thay phần lõi, giữ nguyên runtime/model, `config.json`, profile Edge, prompt, logs và toàn bộ thư mục ảnh. Trước khi thay file, updater xác minh SHA-256 và tạo backup để tự khôi phục nếu có lỗi.
